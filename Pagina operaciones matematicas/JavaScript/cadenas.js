class Cadenas{
    ...
    bus(){
        ...
        if (posicion !== -1){
            resp.textContent = `La subcadena "${subcadena}" se encuentra en la posicion ${posicion}`
        } else{
            resp.textContent = `La subcadena no se encontro en la frase`
        }
    ...
    insertarSubcadena(){
        ...
        if (posicion >= 0 && posicion <= cadena.length){ 
            ...
            let resp = document.getElementById("resp")
            resp.textContent = `Posicion no valida`
        }
    }
    eliminarSubcadena(){
        let cadena = document.getElementById("cadena").value
